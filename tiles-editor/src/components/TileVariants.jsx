    ...
    return tiles.map((tile, originalIndex) => {
      ...
      if (tile.rotationEnabled) {
        for (let i = 1; i < 4; i++) {
          ...
          if (!variants.some(v => 
            JSON.stringify(v.grid) === JSON.stringify(rotated.grid)
          )) {
            variants.push({
              grid: rotated.grid,
              type: "rotation",
      ...
      if (tile.mirrorEnabled) {
        ...
        if (!variants.some(v => 
          JSON.stringify(v.grid) === JSON.stringify(mirrored.grid)
        )) {
          variants.push({
            grid: mirrored.grid,
            type: "mirror",
        ...
        if (tile.rotationEnabled) {
          for (let i = 1; i < 4; i++) {
            const rotatedMirror = rotateTile({ ...tile, grid: mirrored.grid }, i);
        ...
        <div key={index} className="original-tile-section">
          ...
          <div className="tile-settings">
            <span className="tile-setting">
              Rotation: {item.originalTile.rotationEnabled ? "Enabled" : "Disabled"}
            </span>
            <span className="tile-setting">
              Mirroring: {item.originalTile.mirrorEnabled ? "Enabled" : "Disabled"}
            </span>
            <span className="tile-setting">
          ...
          <div className="variants-grid">
            {item.variants.map((variant, variantIndex) => (
              <div key={variantIndex} className={`variant-item ${variant.type}`}>
