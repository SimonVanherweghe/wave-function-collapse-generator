...
describe("WFC Visualization and Reset", () => {
  it("updates UI as cells become collapsed", async () => {
    ...
    await waitFor(() => {
      ...
      const collapsedCells = updatedCells.filter(
        (cell) => cell.textContent !== "7"
      );
  ...
  it("resets the grid to its initial state after clicking Reset", async () => {
    ...
    await waitFor(() => {
      ...
      const collapsedCells = cellsAfterRun.filter(
        (cell) => cell.textContent !== "7"
      );
      expect(collapsedCells.length).toBeGreaterThan(0);
    });
    ...
    await waitFor(() => {
      ...
      // Now, every cell should display the initial possibility count, "7".
